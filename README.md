...
>   또한, POST 메서드로만 구현되어 있던 '좋아요' API를 RESTful하게 POST와 DELETE로 분리했습니다. 유니크 키와 트랜잭션을 활용하여 데이터의 존재 여부에 따라 중복 데이터를 판별하고 적절한 처리 방법을 적용함으로써 데이터베이스의 중복 등록 문제를 해결하였습니다.
